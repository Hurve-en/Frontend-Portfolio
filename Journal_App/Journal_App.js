/*
  ...
  This file handles all app behavior:
  - Load/save entries from browser storage
  - Display entries list with search and filters
  - Open/edit/delete entries
...
(() => {
  // Setup: Mood options and app variables
  ...
  let entries = []; // All saved entries
  let currentId = null; // ID of entry being edited
  ...
  // Quick function to get HTML elements by ID
  ...
  // Display functions - update what appears on screen
  ...
  // Create mood filter buttons (emotion emojis) below search
  ...
  function getActiveMoodFilters() {
    return Array.from(
      moodFiltersWrap.querySelectorAll(".mood-chip.active"),
    ).map((b) => b.dataset.mood);
  }

  // Display the list of entries, filtered by search and filters
  function renderEntriesList() {
    ...
    filtered.forEach((e) => {
      ...
      card.addEventListener("keydown", (ev) => {
        if (ev.key === "Enter") openDetail(e.id);
      });
  ...
  // Show the editor form for a new or existing entry
  ...
  // Show the detail view to read an entry
  ...
  function exportEntriesToPDF(list, filename = "journal-export.pdf") {
    try {
      ...
      list.forEach((e, idx) => {
        ...
        doc.setFontSize(10);
        doc.text(
          `${mood ? mood.emoji + " " + mood.label + " • " : ""}${new Date(e.createdAt).toLocaleString()}`,
          left,
          y,
        );
        y += 16;
  ...
  function wireListeners() {
    ...
    saveBtn.addEventListener("click", (ev) => {
      ...
      if (currentId) updateEntry(currentId, data);
      else createEntry(data);
      renderEditorFor(null);
    });
    saveBtnBottom.addEventListener("click", (ev) => {
      ev.preventDefault();
      const data = gatherForm();
      if (currentId) updateEntry(currentId, data);
      else createEntry(data);
    ...
    cancelBtnBottom.addEventListener("click", (ev) => {
      ev.preventDefault();
      renderEditorFor(null);
    });
