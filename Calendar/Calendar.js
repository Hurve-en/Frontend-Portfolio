// Calendar logic for the planner page.

/* Minimalist Planner script
   - switch months
...
// Selected day and event list rendering
...
// Event modal: add or edit events
  ...
  if (editingEvent) {
    ...
    if (idx !== -1) {
      ...
      if (editingEvent.date !== dateStr) {
        arr.splice(idx, 1);
        if (!appState.events[dateStr]) appState.events[dateStr] = [];
        appState.events[dateStr].push(
          arr[idx] || { id: editingEvent.id, title, time, desc },
        );
      }
    }
  } else {
...
// Calendar navigation and add-event controls
